...
export default {
  ...
  methods: {
    onLoad() {
      ...
      for (let i = 0; i < result.loanTerm * 12; i++) {
        if (this.repaymentMethod == 'a') {
          this.detailList = equalInterestDetail(result.loanTotal, result.loanTerm, result.loanRate);
        } else if (this.repaymentMethod == 'b') {
          this.detailList = equalPrincipalDetail(result.loanTotal, result.loanTerm, result.loanRate);
        }
      }
